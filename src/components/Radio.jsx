...
function RadioSiNoAlt(props) {
    ...
    return (
        <div className="checks-input mt-2">
        <label className="input-label mb-1">{ props.label }</label>
            <input name={props.id} type="radio" checked={props.checked} onChange={ (e) => props.onChange() }
                className="appearance-none h-3 w-3 border-2 rounded-full border-input checked:bg-input"/>
            <label htmlFor={props.id + '_yes'} className="text-gray-500 text-xm me-20 ps-1">Sí</label>
            <input name={props.id} type="radio" checked={!props.checked} onChange={ (e) => props.onChange() }
                className="appearance-none h-3 w-3 border-2 rounded-full border-input checked:bg-input"  />
            <label htmlFor={props.id + '_no'} className="text-gray-500 text-xm ps-1">No</label>
        </div>
    )
}
export {RadioSiNoAlt} ;
